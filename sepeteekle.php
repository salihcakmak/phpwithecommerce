...
if(isset($_SESSION["Kullanici"])){
	...
	if(($GelenID!="") and ($GelenVaryantID!="")){
		$KullanicininSepetKontrolSorgu	=	$dbConnection->prepare("SELECT * FROM sepet WHERE UyeId = ? ORDER BY id DESC LIMIT 1");
		...
		if($KullanicininSepetSayisi>0){
			...
			if($UrunSepetSayisi>0){
				...
				$UrunGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE sepet SET UrunAdedi = ? WHERE id = ? AND UyeId = ? AND UrunId = ? LIMIT 1");
				...
				$SonIdDegeri			=	$dbConnection->lastInsertId();
					...
					if($UrunEklemeSayisi>0){
						$SiparisNumarasiniGuncelleSorgusu		=	$dbConnection->prepare("UPDATE sepet SET SepetNumarasi = ? WHERE UyeId = ?");
