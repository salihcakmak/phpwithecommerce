...
$ToplamKayitSayisiSorgusu				=	$dbConnection->prepare("SELECT DISTINCT SiparisNumarasi FROM siparisler WHERE OnayDurumu = ? AND KargoDurumu = ? ORDER BY id DESC");
...
<table width="760" align="center" border="0" cellpadding="0" cellspacing="0">
	...
	<?php
	$SiparisNumaralariSorgusu		=	$dbConnection->prepare("SELECT DISTINCT SiparisNumarasi FROM siparisler WHERE OnayDurumu = ? AND KargoDurumu = ? ORDER BY id DESC LIMIT $SayfalamayaBaslanacakKayitSayisi, $SayfaBasinaGosterilecekKayitSayisi");
	...
	if($SiparisNumaralariSayisi>0){
		foreach($SiparisNumaralariKayitlari as $SiparisNumaralariSatirlar){
			$SiparislerSorgusu	=	$dbConnection->prepare("SELECT * FROM siparisler WHERE SiparisNumarasi = ? AND OnayDurumu = ? AND KargoDurumu = ?");
				...
				<td colspan="2" style="border-bottom: 1px dashed #CCCCCC;" valign="top"><table width="750" align="right" border="0" cellpadding="0" cellspacing="0">
					<tr height="30">
						...
						<td align="left" width="75"><table width="75" align="right" border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td width="25"><a href="index.php?SKD=0&SKI=109&SiparisNo=<?php echo DonusumleriGeriDondur($SiparisNumaralariSatirlar["SiparisNumarasi"]); ?>"><img src="../images/DokumanKirmiziKalemli20x20.png" border="0" style="margin-top: 5px;"></a></td>
