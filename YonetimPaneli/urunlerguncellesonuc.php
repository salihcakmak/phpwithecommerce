...
if(isset($_SESSION["Yonetici"])){
	...
	if(($GelenUrunMenusu!="") and ($GelenUrunAdi!="") and ($GelenUrunFiyati!="") and ($GelenParaBirimi!="") and ($GelenKdvOrani!="") and ($GelenKargoUcreti!="") and ($GelenUrunAciklamasi!="") and ($GelenVaryantBasligi!="") and ($GelenVaryantAdi1!="") and ($GelenStokAdedi1!="")){
		$UrunSorgusu	=	$dbConnection->prepare("SELECT * FROM urunler WHERE id = ? LIMIT 1");
		...
		if($UrunKontrol>0){
			...
			if($MenuTuruKontrol>0){
				$UrunGuncellemeSorgusu		=	$dbConnection->prepare("UPDATE urunler SET MenuId = ?, UrunAdi = ?, UrunFiyati = ?, ParaBirimi = ?, KdvOrani = ?, UrunAciklamasi = ?, VaryantBasligi = ?, KargoUcreti = ? WHERE id = ? LIMIT 1");
				...
				if(($GelenResim1["name"]!="") and ($GelenResim1["type"]!="") and ($GelenResim1["tmp_name"]!="") and ($GelenResim1["error"]==0) and ($GelenResim1["size"]>0)){
					...
					$BirinciResimYukle	=	new \Verot\Upload\Upload($GelenResim1, "tr-TR");
						if($BirinciResimYukle->uploaded){
						   $BirinciResimYukle->mime_check			=	true;
						   ...
						   $BirinciResimYukle->process($VerotIcinKlasorYolu.$ResimKlasoru);
							...
							if($BirinciResimYukle->processed){
								$SilinecekBirinciResimYolu		=	"../images/".$ResimKlasoru.$UrunBilgisi["UrunResmiBir"];
								unlink($SilinecekBirinciResimYolu);

								$BirinciResimGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunler SET UrunResmiBir = ? WHERE id = ? LIMIT 1");
				...
				if(($GelenResim2["name"]!="") and ($GelenResim2["type"]!="") and ($GelenResim2["tmp_name"]!="") and ($GelenResim2["error"]==0) and ($GelenResim2["size"]>0)){
					...
					$IkinciResimYukle	=	new \Verot\Upload\Upload($GelenResim2, "tr-TR");
						if($IkinciResimYukle->uploaded){
						   $IkinciResimYukle->mime_check			=	true;
						   ...
						   $IkinciResimYukle->process($VerotIcinKlasorYolu.$ResimKlasoru);
							...
							if($IkinciResimYukle->processed){
								$SilinecekIkinciResimYolu		=	"../images/".$ResimKlasoru.$UrunBilgisi["UrunResmiIki"];
								unlink($SilinecekIkinciResimYolu);

								$IkinciResimGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunler SET UrunResmiIki = ? WHERE id = ? LIMIT 1");
				...
				if(($GelenResim3["name"]!="") and ($GelenResim3["type"]!="") and ($GelenResim3["tmp_name"]!="") and ($GelenResim3["error"]==0) and ($GelenResim3["size"]>0)){
					...
					$UcuncuResimYukle	=	new \Verot\Upload\Upload($GelenResim3, "tr-TR");
						if($UcuncuResimYukle->uploaded){
						   $UcuncuResimYukle->mime_check			=	true;
						   ...
						   $UcuncuResimYukle->process($VerotIcinKlasorYolu.$ResimKlasoru);
							...
							if($UcuncuResimYukle->processed){
								$SilinecekUcuncuResimYolu		=	"../images/".$ResimKlasoru.$UrunBilgisi["UrunResmiUc"];
								unlink($SilinecekUcuncuResimYolu);

								$UcuncuResimGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunler SET UrunResmiUc = ? WHERE id = ? LIMIT 1");
				...
				if(($GelenResim4["name"]!="") and ($GelenResim4["type"]!="") and ($GelenResim4["tmp_name"]!="") and ($GelenResim4["error"]==0) and ($GelenResim4["size"]>0)){
					...
					$DorduncuResimYukle	=	new \Verot\Upload\Upload($GelenResim4, "tr-TR");
						if($DorduncuResimYukle->uploaded){
						   $DorduncuResimYukle->mime_check			=	true;
						   ...
						   $DorduncuResimYukle->process($VerotIcinKlasorYolu.$ResimKlasoru);
							...
							if($DorduncuResimYukle->processed){
								$SilinecekDorduncuResimYolu		=	"../images/".$ResimKlasoru.$UrunBilgisi["UrunResmiDort"];
								unlink($SilinecekDorduncuResimYolu);

								$DorduncuResimGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunler SET UrunResmiDort = ? WHERE id = ? LIMIT 1");
				...
				$VaryantlarSorgusu	=	$dbConnection->prepare("SELECT * FROM urunvaryantlari WHERE UrunId = ?");
				...
				if(array_key_exists(0, $VaryantIsimDizisi)){
					if(($GelenVaryantAdi1!="") and ($GelenStokAdedi1!="")){
						$BirinciVaryantGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunvaryantlari SET VaryantAdi = ?, StokAdedi = ? WHERE UrunId = ? AND VaryantAdi = ? LIMIT 1");
				...
				if(array_key_exists(1, $VaryantIsimDizisi)){
					if(($GelenVaryantAdi2!="") and ($GelenStokAdedi2!="")){
						$IkinciVaryantGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunvaryantlari SET VaryantAdi = ?, StokAdedi = ? WHERE UrunId = ? AND VaryantAdi = ? LIMIT 1");
					...
					if(($GelenVaryantAdi2!="") and ($GelenStokAdedi2!="")){
						$IkinciVaryantEklemeSorgusu		=	$dbConnection->prepare("INSERT INTO urunvaryantlari (UrunId, VaryantAdi, StokAdedi) values (?, ?, ?)");
				...
				if(array_key_exists(2, $VaryantIsimDizisi)){
					if(($GelenVaryantAdi3!="") and ($GelenStokAdedi3!="")){
						$UcuncuVaryantGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunvaryantlari SET VaryantAdi = ?, StokAdedi = ? WHERE UrunId = ? AND VaryantAdi = ? LIMIT 1");
					...
					if(($GelenVaryantAdi3!="") and ($GelenStokAdedi3!="")){
						$UcuncuVaryantEklemeSorgusu		=	$dbConnection->prepare("INSERT INTO urunvaryantlari (UrunId, VaryantAdi, StokAdedi) values (?, ?, ?)");
				...
				if(array_key_exists(3, $VaryantIsimDizisi)){
					if(($GelenVaryantAdi4!="") and ($GelenStokAdedi4!="")){
						$DorduncuVaryantGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunvaryantlari SET VaryantAdi = ?, StokAdedi = ? WHERE UrunId = ? AND VaryantAdi = ? LIMIT 1");
					...
					if(($GelenVaryantAdi4!="") and ($GelenStokAdedi4!="")){
						$DorduncuVaryantEklemeSorgusu		=	$dbConnection->prepare("INSERT INTO urunvaryantlari (UrunId, VaryantAdi, StokAdedi) values (?, ?, ?)");
				...
				if(array_key_exists(4, $VaryantIsimDizisi)){
					if(($GelenVaryantAdi5!="") and ($GelenStokAdedi5!="")){
						$BesinciVaryantGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunvaryantlari SET VaryantAdi = ?, StokAdedi = ? WHERE UrunId = ? AND VaryantAdi = ? LIMIT 1");
					...
					if(($GelenVaryantAdi5!="") and ($GelenStokAdedi5!="")){
						$BesinciVaryantEklemeSorgusu		=	$dbConnection->prepare("INSERT INTO urunvaryantlari (UrunId, VaryantAdi, StokAdedi) values (?, ?, ?)");
				...
				if(array_key_exists(5, $VaryantIsimDizisi)){
					if(($GelenVaryantAdi6!="") and ($GelenStokAdedi6!="")){
						$AltinciVaryantGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunvaryantlari SET VaryantAdi = ?, StokAdedi = ? WHERE UrunId = ? AND VaryantAdi = ? LIMIT 1");
					...
					if(($GelenVaryantAdi6!="") and ($GelenStokAdedi6!="")){
						$AltinciVaryantEklemeSorgusu		=	$dbConnection->prepare("INSERT INTO urunvaryantlari (UrunId, VaryantAdi, StokAdedi) values (?, ?, ?)");
				...
				if(array_key_exists(6, $VaryantIsimDizisi)){
					if(($GelenVaryantAdi7!="") and ($GelenStokAdedi7!="")){
						$YedinciVaryantGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunvaryantlari SET VaryantAdi = ?, StokAdedi = ? WHERE UrunId = ? AND VaryantAdi = ? LIMIT 1");
					...
					if(($GelenVaryantAdi7!="") and ($GelenStokAdedi7!="")){
						$YedinciVaryantEklemeSorgusu		=	$dbConnection->prepare("INSERT INTO urunvaryantlari (UrunId, VaryantAdi, StokAdedi) values (?, ?, ?)");
				...
				if(array_key_exists(7, $VaryantIsimDizisi)){
					if(($GelenVaryantAdi8!="") and ($GelenStokAdedi8!="")){
						$SekizinciVaryantGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunvaryantlari SET VaryantAdi = ?, StokAdedi = ? WHERE UrunId = ? AND VaryantAdi = ? LIMIT 1");
					...
					if(($GelenVaryantAdi8!="") and ($GelenStokAdedi8!="")){
						$SekizinciVaryantEklemeSorgusu		=	$dbConnection->prepare("INSERT INTO urunvaryantlari (UrunId, VaryantAdi, StokAdedi) values (?, ?, ?)");
				...
				if(array_key_exists(8, $VaryantIsimDizisi)){
					if(($GelenVaryantAdi9!="") and ($GelenStokAdedi9!="")){
						$DokuzuncuVaryantGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunvaryantlari SET VaryantAdi = ?, StokAdedi = ? WHERE UrunId = ? AND VaryantAdi = ? LIMIT 1");
					...
					if(($GelenVaryantAdi9!="") and ($GelenStokAdedi9!="")){
						$DokuzuncuVaryantEklemeSorgusu		=	$dbConnection->prepare("INSERT INTO urunvaryantlari (UrunId, VaryantAdi, StokAdedi) values (?, ?, ?)");
				...
				if(array_key_exists(9, $VaryantIsimDizisi)){
					if(($GelenVaryantAdi10!="") and ($GelenStokAdedi10!="")){
						$OnuncuVaryantGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE urunvaryantlari SET VaryantAdi = ?, StokAdedi = ? WHERE UrunId = ? AND VaryantAdi = ? LIMIT 1");
					...
					if(($GelenVaryantAdi10!="") and ($GelenStokAdedi10!="")){
						$OnuncuVaryantEklemeSorgusu		=	$dbConnection->prepare("INSERT INTO urunvaryantlari (UrunId, VaryantAdi, StokAdedi) values (?, ?, ?)");
