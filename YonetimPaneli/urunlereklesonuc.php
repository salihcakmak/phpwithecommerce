...
if(isset($_SESSION["Yonetici"])){
	...
	if(($GelenUrunMenusu!="") and ($GelenUrunAdi!="") and ($GelenUrunFiyati!="") and ($GelenParaBirimi!="") and ($GelenKdvOrani!="") and ($GelenKargoUcreti!="") and ($GelenUrunAciklamasi!="") and ($GelenVaryantBasligi!="") and ($GelenVaryantAdi1!="") and ($GelenStokAdedi1!="") and ($GelenResim1["name"]!="") and ($GelenResim1["type"]!="") and ($GelenResim1["tmp_name"]!="") and ($GelenResim1["error"]==0) and ($GelenResim1["size"]>0)){
		...
		if($MenuTuruKontrol>0){
			...
			$UrunEklemeSorgusu		=	$dbConnection->prepare("INSERT INTO urunler (MenuId, UrunTuru, UrunAdi, UrunFiyati, ParaBirimi, KdvOrani, UrunAciklamasi, UrunResmiBir, VaryantBasligi, KargoUcreti, Durumu) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			...
			if($UrunEklemeKontrol>0){
				$SonEklenenUrununIDsi		=	$dbConnection->lastInsertId();
				...
				$MenuUrunSayisiGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE menuler SET UrunSayisi=UrunSayisi+1 WHERE id = ? LIMIT 1");
				...
				if($MenuUrunSayisiGuncellemeKontrol>0){
					$BirinciVaryantEklemeSorgusu		=	$dbConnection->prepare("INSERT INTO urunvaryantlari (UrunId, VaryantAdi, StokAdedi) values (?, ?, ?)");
