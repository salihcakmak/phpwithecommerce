...
if(isset($_SESSION["Yonetici"])){
	...
	if(($GelenKullaniciAdi!="") and ($GelenSifre!="") and ($GelenIsimSoyisim!="") and ($GelenEmailAdresi!="") and ($GelenTelefonNumarasi!="")){	
		$YoneticiKontrolSorgusu		=	$dbConnection->prepare("SELECT * FROM yoneticiler WHERE KullaniciAdi = ? OR EmailAdresi = ?");
		...
		if($Kontrol>0){
			...
			$YoneticiEklemeSorgusu		=	$dbConnection->prepare("INSERT INTO yoneticiler (KullaniciAdi, Sifre, IsimSoyisim, EmailAdresi, TelefonNumarasi) values (?, ?, ?, ?, ?)");
