...
if(isset($_SESSION["Yonetici"])){
	...
	if($GelenID!=""){
		$UyeSilmeSorgusu	=	$dbConnection->prepare("UPDATE uyeler SET SilinmeDurumu = ? WHERE id = ? LIMIT 1");
		$UyeSilmeSorgusu->execute([1, $GelenID]);
		$UyeSilmeKontrol	=	$UyeSilmeSorgusu->rowCount();

		if($UyeSilmeKontrol>0){
			$SepetSilmeSorgusu		=	$dbConnection->prepare("DELETE FROM sepet WHERE UyeId = ?");
