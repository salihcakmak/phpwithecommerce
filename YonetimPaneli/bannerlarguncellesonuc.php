...
if(isset($_SESSION["Yonetici"])){
	...
	if(($GelenID!="") and ($GelenBannerAlani!="") and ($GelenBannerAdi!="")){
		$BannerResmiSorgusu		=	$dbConnection->prepare("SELECT * FROM bannerlar WHERE id = ? LIMIT 1");
		...
		if($GelenBannerAlani==$BannerBilgisi["BannerAlani"]){		
			$BannerGuncellemeSorgusu		=	$dbConnection->prepare("UPDATE bannerlar SET BannerAlani = ?, BannerAdi = ? WHERE id = ? LIMIT 1");
			...
			if(($GelenBannerResmi["name"]!="") and ($GelenBannerResmi["type"]!="") and ($GelenBannerResmi["tmp_name"]!="") and ($GelenBannerResmi["error"]==0) and ($GelenBannerResmi["size"]>0)){
				$SilinecekDosyaYolu		=	"../images/".$BannerBilgisi["BannerResmi"];
					...
					if($BannerYukle->uploaded){
					   ...
					   $BannerYukle->process($VerotIcinKlasorYolu);
						...
						if($BannerYukle->processed){
							$BannerResmiGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE bannerlar SET BannerResmi = ? WHERE id = ? LIMIT 1");
				...
				$BannerYukle	=	new \Verot\Upload\Upload($GelenBannerResmi, "tr-TR");
					if($BannerYukle->uploaded){
					   ...
					   $BannerYukle->process($VerotIcinKlasorYolu);
						...
						if($BannerYukle->processed){
							$BannerResmiGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE bannerlar SET BannerAlani = ?, BannerAdi = ?, BannerResmi = ? WHERE id = ? LIMIT 1");
