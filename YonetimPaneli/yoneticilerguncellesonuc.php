...
if(isset($_SESSION["Yonetici"])){
	...
	if(($GelenID!="") and ($GelenIsimSoyisim!="") and ($GelenEmailAdresi!="") and ($GelenTelefonNumarasi!="")){	
		$YoneticininMevcutSifreSorgusu		=	$dbConnection->prepare("SELECT * FROM yoneticiler WHERE id = ? LIMIT 1");
		...
		if($YoneticininMevcutSifreKontrolu>0){
			...
			$YoneticiGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE yoneticiler SET IsimSoyisim = ?, Sifre = ?, EmailAdresi = ?, TelefonNumarasi = ? WHERE id = ? LIMIT 1");
