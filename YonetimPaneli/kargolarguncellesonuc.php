...
if(isset($_SESSION["Yonetici"])){
	...
	if(($GelenID!="") and ($GelenKargoFirmasiAdi!="")){
		$KargoGuncellemeSorgusu		=	$dbConnection->prepare("UPDATE kargofirmalari SET KargoFirmasiAdi = ? WHERE id = ? LIMIT 1");
		...
		if(($GelenKargoFirmasiLogosu["name"]!="") and ($GelenKargoFirmasiLogosu["type"]!="") and ($GelenKargoFirmasiLogosu["tmp_name"]!="") and ($GelenKargoFirmasiLogosu["error"]==0) and ($GelenKargoFirmasiLogosu["size"]>0)){
			$KargoResmiSorgusu		=	$dbConnection->prepare("SELECT * FROM kargofirmalari WHERE id = ? LIMIT 1");
			...
			$SilinecekDosyaYolu		=	"../images/".$ResimBilgisi["KargoFirmasiLogosu"];
			...
			$KargoLogosuYukle	=	new \Verot\Upload\Upload($GelenKargoFirmasiLogosu, "tr-TR");
				if($KargoLogosuYukle->uploaded){
				   $KargoLogosuYukle->mime_check				=	true;
				   ...
				   $KargoLogosuYukle->process($VerotIcinKlasorYolu);
					...
					if($KargoLogosuYukle->processed){
						$KargoResmiGuncellemeSorgusu	=	$dbConnection->prepare("UPDATE kargofirmalari SET KargoFirmasiLogosu = ? WHERE id = ? LIMIT 1");
